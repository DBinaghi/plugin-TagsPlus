...
class TagsPlus_IndexController extends Omeka_Controller_AbstractActionController
{
	...
	public function changeCaseAction()
	{
		...
		$mode = $_POST['mode'] ?? '';
		if (!in_array($mode, array('upper', 'lower', 'title', 'sentence'))) {
			$this->getResponse()->setHttpResponseCode(400);
			return;
		}
		...
		foreach ($tags as $tag) {
			...
			switch ($mode) {
				...
				case 'sentence': $newName = mb_strtoupper(mb_substr($oldName, 0, 1, 'UTF-8'), 'UTF-8') . mb_strtolower(mb_substr($oldName, 1, mb_strlen($oldName) - 1, 'UTF-8'), 'UTF-8'); break;
			}
